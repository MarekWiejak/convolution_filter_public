...
int main(int argc, char** argv) {
    try {
        ...
        my_graph.init();
        my_graph.update(my_graph.coeffs, float2fixed_coeff<10, 16>(kData).data(), 16);
        ...
        for (int i = 0; i < iterations; i++) {
			...
			for (int i=0; i< tiles_sz[0] * tiles_sz[1]; i++){
				std::cout << "Starting " << i << " sub_iteration" << std::endl;
				my_graph.run(1);
				my_graph.wait();
				std::cout << "Done " << i << " sub_iteration" << std::endl;
			}
//            my_graph.run(tiles_sz[0] * tiles_sz[1]);
